...
def plot_ssil_sg(ax, xlabel, ylabel, legend=False):
...
def plot_ssil(ax, xlabel, ylabel, metric, legend=False):
    assert "WANDB_API_KEY" in os.environ, "WANDB_API_KEY is not set"
    wandb_api_key = os.environ["WANDB_API_KEY"]
    wandb_entity = "stability-gap"
    wandb_project = "cl-teacher-adaptation-src"
    ...
    plot.set_title(title)
    plot.set_xlabel(xlabel)
    plot.set_xticks(range(num_tasks))
...
def main():
    ...
    plot_ssil_sg(axes[0], xlabel="Finished Task", ylabel=None)
    plot_ssil(axes[1], xlabel="Finished Task", ylabel=None, metric="cont_eval/wc_acc")
    plot_ssil(axes[2], xlabel='Finished Task', ylabel=None, metric="cont_eval/min_acc", legend=True)

    output_path_png = output_dir / "apx_merged_ssil_sg.png"
    output_path_pdf = output_dir / "apx_merged_ssil_sg.pdf"
    plt.tight_layout()
