...
def plot_lwf_sg(ax, xlabel, ylabel, legend=False):
...
def plot_lwf(ax, xlabel, ylabel, metric, legend=False):
    assert "WANDB_API_KEY" in os.environ, "WANDB_API_KEY is not set"
    wandb_api_key = os.environ["WANDB_API_KEY"]
    wandb_entity = "stability-gap"
    wandb_project = "cl-teacher-adaptation-src"
...
def main():
    ...
    plot_lwf_sg(axes[0], xlabel="Finished Task", ylabel=None)
    plot_lwf(axes[1], xlabel="Finished Task", ylabel=None, metric="cont_eval/wc_acc")
    plot_lwf(axes[2], xlabel='Finished Task', ylabel=None, metric="cont_eval/min_acc", legend=True)

    output_path_png = output_dir / "apx_merged_lwf_sg.png"
    output_path_pdf = output_dir / "apx_merged_lwf_sg.pdf"
    plt.tight_layout()
