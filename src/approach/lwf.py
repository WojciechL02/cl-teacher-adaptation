...
class Appr(Inc_Learning_Appr):
        ...
        for images, targets in trn_loader:
            ...
            if t > 0:
                if self.ctt or self.cbnt or self.ta:
                    ...
                    self.model_old.eval()
                targets_old = self.model_old(images)  # , is_eval=True
                if self.ctt or self.cbnt:
    ...
    def eval(self, t, val_loader, log_partial_loss=False):
        ...
        if log_partial_loss:
            ...
            self.logger.log_scalar(
                task=None, iter=None, name="loss_kd", value=final_loss_kd, group="valid"
            )
            self.logger.log_scalar(
                task=None, iter=None, name="loss_ce", value=final_loss_ce, group="valid"
            )
