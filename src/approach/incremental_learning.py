...
class Inc_Learning_Appr:
    ...
    def __init__(
        ...
        no_learning=False,
        log_grad_norm=True,
        best_prototypes=False,
        slca=False,
        cont_eval=False,
        umap_latent=False,
        last_head_analysis=False,
        prototype_head_similarity=False,
    ):
        ...
        self.slca = slca
        self.evaluator = ContinualEvaluator(self, prototype_head_similarity, self.tst_loader, enabled=cont_eval)
    ...
    def _get_optimizer(self):
        ...
        if self.slca:
            ...
            return torch.optim.SGD(
                network_params, lr=self.lr, weight_decay=self.wd, momentum=self.momentum
            )
        return torch.optim.SGD(
    ...
    def _get_scheduler(self):
        if self.scheduler_type == "linear":
            return torch.optim.lr_scheduler.LinearLR(
                ...
                start_factor=1.0,
                end_factor=0.01,
                total_iters=self.nepochs,
            )
        elif self.scheduler_type == "multistep":
            return torch.optim.lr_scheduler.MultiStepLR(
                optimizer=self.optimizer, milestones=[30, 60, 80], gamma=0.1
            )
        elif self.scheduler_type == "cosine":
            return torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer=self.optimizer, T_max=self.nepochs, eta_min=self.lr * 0.01
            )
        else:
            return None
        ...
        with torch.no_grad():
            for i, loader in enumerate(loaders):
                for images, targets in loader:
                    ...
                    _, backbone_feats, proj_feats, final_feats = self.model(
                        images,
                        return_features=True,
                        return_proj_output=True,
                    )
                    backbone_list.append(backbone_feats)
                    proj_list.append(proj_feats)
                    final_list.append(final_feats)
                    labels.append(targets)

        backbone_vis = self._umap_projection(
            torch.cat(backbone_list, dim=0), torch.cat(labels, dim=0)
        )
        proj_vis = self._umap_projection(
            torch.cat(proj_list, dim=0), torch.cat(labels, dim=0)
        )
        final_vis = self._umap_projection(
            torch.cat(final_list, dim=0), torch.cat(labels, dim=0)
        )
        self.logger.log_latent_vis(
            group="umap_backbone", task=t, epoch=e, data=backbone_vis
        )
        self.logger.log_latent_vis(
            group="umap_projector", task=t, epoch=e, data=proj_vis
        )
        self.logger.log_latent_vis(
            group="umap_final_latent", task=t, epoch=e, data=final_vis
        )
        ...
        if self.warmup_epochs and t > 0:
            ...
            if self.warmup_scheduler.lower() == "constant":
                ...
                scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    optimizer, T_max=self.warmup_epochs, eta_min=0
                )
        ...
        with torch.no_grad():
            for i, loader in enumerate(loaders):
                ...
                for images, targets in loader:
                    ...
                    hits_taw, hits_tag = self.classifier.classify(
                        t, outputs, feats, targets
                    )
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        for e in range(self.nepochs):
            ...
            if self.eval_on_train:
                train_loss, train_acc, _ = self.eval(
                    t, trn_loader, log_partial_loss=False
                )
                clock2 = time.time()
                print(
            ...
            if self.umap_latent and (
                is_end_first_task or is_first_epoch or is_middle_epoch or is_last_epoch
            ):
                self._umap_latent_space(t, e)
            if self.model.projector_type is not None and self.model.projector_type != "double" and (e == self.nepochs - 1):
                self._umap_latent_space(t, e)
            # ========================================
        ...
        if self.last_head_analysis and t > 0 and self.warmup_epochs > 0:
            head_after_wu_vect = torch.nn.utils.parameters_to_vector(
                head_after_wu.parameters()
            ).unsqueeze(0)
            head_curr_vect = torch.nn.utils.parameters_to_vector(
            ...
            cos_sim = torch.nn.functional.cosine_similarity(
                head_after_wu_vect, head_curr_vect
            )
            self.logger.log_scalar(
                task=None,
                iter=None,
    ...
    def _log_heads_activation_statistics(self, t):
        ...
        with torch.no_grad():
            ...
            for task_id, loader in enumerate(loaders):
                actv = [
                    torch.zeros((len(loader.dataset), self.model.heads[i].out_features))
                ...
                self.logger.log_scalar(
                    ...
                    iter=None,
                    name="Max",
                    group="Head activations",
                    value=final_max.item(),
                )
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            if self.model.projector_type is not None:
                if self.model.projector_type == "double":
                    outputs, backbone_feat, proj_output, proj_output2, _, _ = self.model(
                        images.to(self.device),
                        return_features=True,
                        return_proj_output=True,
                    )
                else:
                    outputs, backbone_feat, proj_output, _ = self.model(
                        images.to(self.device),
                        return_features=True,
                        return_proj_output=True,
                    )
            ...
            if t == 0 or not self.no_learning:
                ...
                # ========== LOG PROJECTOR GRAD NORM===========
                if self.model.projector_type is not None:
                    g_norm = self.model.projector.get_grad_norm()
                    self.logger.log_scalar(
                        task=t,
                        iter=None,
                        name="projector",
                    ...
                    w_norm = self.model.projector.get_weights_norm()
                    self.logger.log_scalar(
                        task=t,
                        iter=None,
                        name="projector",
                    ...
                    proj_out_var = proj_output.var(dim=1).mean()
                    self.logger.log_scalar(
                        task=t,
                        iter=None,
                        name="projector",
                        value=proj_out_var.item(),
                        group="Output_Var",
                    )
                    cos_sims = torch.nn.functional.cosine_similarity(
                        backbone_feat, proj_output
                    )
                    self.logger.log_scalar(
                        task=t,
                        iter=None,
                        name="features_cos_sim",
                        value=cos_sims.mean().item(),
                        group="Backbone_Projector",
                    )
                    if self.model.projector_type == "double":
                        proj_out_var = proj_output2.var(dim=1).mean()
                        self.logger.log_scalar(
                            task=t,
                            iter=None,
                            name="projector2",
                        ...
                        cos_sims = torch.nn.functional.cosine_similarity(
                            backbone_feat, proj_output2
                        )
                        self.logger.log_scalar(
                            task=t,
                            iter=None,
                            name="features_cos_sim2",
                            value=cos_sims.mean().item(),
                            group="Backbone_Projector",
                        )
                # ====================================
    ...
    def eval(self, t, val_loader, log_partial_loss=False):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                outputs, feats = self.model(images, return_features=True, is_eval=True)
