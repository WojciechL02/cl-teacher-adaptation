...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=60, lr=0.5, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
        ...
        self.exemplar_means = []
        self.loss_func = SupConLoss(temperature, device)
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            y1, _ = self.model(images1.to(self.device), return_features=True)
            y2, _ = self.model(images2.to(self.device), return_features=True)
            y = torch.cat([y1.unsqueeze(1), y2.unsqueeze(1)], dim=1)
            loss = self.criterion(t, y, targets.to(self.device))
            # Backward
