...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        choices=["full", "mlp", "low_rank", "differential", "double"],
        help="Use residual projector network during training (default=%(default)s)",
    )
    # training args
    parser.add_argument(
        "--approach",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        choices=["linear", "onecycle", "cosine"],
    ...
    parser.add_argument(
        "--prototype-head-similarity",
        action="store_true",
        help="Compute cosine similarity between prototypes and linear head neurons before and after projector ((default=%(default)s))",
    )
    # gridsearch args
    parser.add_argument(
        "--gridsearch-tasks",
    ...
    base_kwargs = dict(
        ...
        slca=args.slca,
        cont_eval=args.cont_eval,
        umap_latent=args.umap_latent,
        log_grad_norm=args.log_grad_norm,
        last_head_analysis=args.last_head_analysis,
        prototype_head_similarity=args.prototype_head_similarity,
    )
    ...
    if args.approach in ["scr", "scr_lb", "mixed"]:
        net = SSL_Net(
            ...
            head_init_mode=args.head_init_mode,
            projector_type=appr_args.projector_type,
        )
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        if args.cm:
            logger.log_result(
                cm(appr, tst_loader[: t + 1], args.num_tasks, appr.device),
                name="cm",
                step=t,
